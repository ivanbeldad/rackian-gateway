...
CHART_DIR="$BUILD_DIR/chart/rackian-gateway"
GCS_PROTOCOL="gs"
GCS_BUCKET="rackian-cloud"

if [ $ENVIRONMENT = "production" ]; then
  RELEASE_NAME="rackian-gateway"
  SECRET_PATH="$CHART_DIR/values.secret.yaml"
  GCS_SECRET="gateway/values.secret.yaml"
  GCS_SECRET_PATH="$GCS_PROTOCOL://$GCS_BUCKET/$GCS_SECRET"
else
  RELEASE_NAME="rackian-gateway-staging"
  SECRET_PATH="$CHART_DIR/values.secret.staging.yaml"
  GCS_SECRET="gateway/values.secret.staging.yaml"
  GCS_SECRET_PATH="$GCS_PROTOCOL://$GCS_BUCKET/$GCS_SECRET"
fi
